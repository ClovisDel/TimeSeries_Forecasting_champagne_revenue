...
data$date %>% 
  format(format = "%Y") %>%
  bartlett.test(data$value, .)
```
Le test est peu significatif, il ne justifie pas une transformation de Box-cox pour éléminer l'hétéroscédasticité de nos valeurs.
...
Après application de l'opérateur $\nabla ^{12}$ on observe la disparition de la tendance, cela est dû à la présence du facteur $I - B$ dans la formule de l'opérateur nabla en saisonnalité.

$$ \nabla ^{12} = I - B^{12} = (1 - B)P(B)$$

> test de tendance : Mann-Kendall Test : <https://www.statology.org/mann-kendall-trend-test-r/> (H0) : There is no trend (H1) : There is a trend
...
D'après le corrélograme de (I-B\^12)Xt on peut déduire : \> La série est stationnaire \> La série présente des valeurs fortes en \^p(1) et \^p(12), donc probablement introduire un MA \> Dans les premiers niveaux on remarque une valeur en forte en q=1, On garde un MA(1) pour le moment.
...
On obient une p_value de 0.7071, le test est non significatif, on ne rejette pas H0. On peut conclure que les résidus εt sont non corrélés.
...
p <- dygraph(don, main = "Affichage de la présivion de l'Arima",ylab = "Revenue vente de champagne") %>%
