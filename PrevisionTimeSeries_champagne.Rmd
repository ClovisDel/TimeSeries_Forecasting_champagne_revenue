...
```{r}
decompose_ts <- decompose(data_ts_train)
plot(decompose_ts)
```
...
A l'oeuil nu on remarque que la série temporelle n'a plus de tendance, mais on vérifie par le test de Mann-Kendall. 
...
```{r}
plot(model1$residuals, main = "Résidus du modèle SARIMA12 (0,0,1)(0,1,1)", xlab = "Année", ylab = "Valeur")
```
```{r}
...
```{r}
MAPE <- mape(data_ts_test, pred$mean)
MAE <- mae(data_ts_test, pred$mean)
MSE <- mse(data_ts_test, pred$mean)
RMSE <- rmse(data_ts_test, pred$mean)
